...
def Plot(cfg):
	...
	regions = None
	if not cfg['snv_map'] is None:
		print "loading region list"
		snv_list = FileFxns.LoadCoordinates(cfg['snv_map'])
		regions = list(snv_list['region'])
	elif not cfg['region'] is None:
		if len(cfg['region'].split(':')) > 1:
			snv_list = pd.DataFrame({'chr': [re.split(':|-',cfg['region'])[0]],'start': [re.split(':|-',cfg['region'])[1]],'end': [re.split(':|-',cfg['region'])[2]],'region': [cfg['region']]})
		else:
			snv_list = pd.DataFrame({'chr': [cfg['region']],'start': ['NA'],'end': ['NA'],'region': [cfg['region']]})
		snv_list['id'] = 'NA'
		regions = list(snv_list['region'])
	...
	if regions is None:
		...
		for r in range(len(snv_list.index)):
			reg = snv_list['region'][r]
			try:
				...
				if len(chunk) > 0:
					if r == 0:
						...
						pvals = pvals.append(chunk)
			print "   processed region " + str(r+1) + "/" + str(len(snv_list.index)) + " (" + str(snv_list['id'][r]) + " " + str(snv_list['region'][r]) + "): " + str(len(chunk)) + " markers added: " + str(len(pvals.index)) + " total"
	...
	if regions is None:
		...
		if cfg['qq'] and freq_col in pvals and cfg['qq_strat']:
			...
			if cfg['ext'] == 'tiff':
				grdevices.tiff(cfg['file'].replace('.gz','') + '.' + cfg['stat'] + '.qq_strat.' + cfg['ext'],width=4,height=4,units="in",bg="white",compression="lzw",res=300)
			elif cfg['ext'] == 'eps':
				grdevices.postscript(cfg['file'].replace('.gz','') + '.' + cfg['stat'] + '.qq_strat.' + cfg['ext'],width=4,height=4,bg="white",horizontal=False)
			else:
				grdevices.pdf(cfg['file'].replace('.gz','') + '.' + cfg['stat'] + '.qq_strat.' + cfg['ext'],width=4,height=4,bg="white")
		...
		if cfg['qq']:
			...
			if cfg['ext'] == 'tiff':
				grdevices.tiff(cfg['file'].replace('.gz','') + '.' + cfg['stat'] + '.qq.' + cfg['ext'],width=4,height=4,units="in",bg="white",compression="lzw",res=300)
			elif cfg['ext'] == 'eps':
				grdevices.postscript(cfg['file'].replace('.gz','') + '.' + cfg['stat'] + '.qq.' + cfg['ext'],width=4,height=4,bg="white",horizontal=False)
			else:
				grdevices.pdf(cfg['file'].replace('.gz','') + '.' + cfg['stat'] + '.qq.' + cfg['ext'],width=4,height=4,bg="white")
			gp = ggplot2.ggplot(df)
			pp = gp + \
					ggplot2.aes_string(x='a',y='b') + \
		...
		if cfg['mht']:
			...
			if cfg['ext'] == 'tiff':
				grdevices.tiff(cfg['file'].replace('.gz','') + '.' + cfg['stat'] + '.mht.' + cfg['ext'],width=12,height=4,units="in",bg="white",compression="lzw",res=300)
			elif cfg['ext'] == 'eps':
				grdevices.postscript(cfg['file'].replace('.gz','') + '.' + cfg['stat'] + '.mht.' + cfg['ext'],width=12,height=4,bg="white",horizontal=False)
			else:
				grdevices.pdf(cfg['file'].replace('.gz','') + '.' + cfg['stat'] + '.mht.' + cfg['ext'],width=12,height=4,bg="white")
		...
		pvals_out.to_csv(cfg['file'].replace('.gz','') + '.' + cfg['stat'] + '.top_results', header=True, index=False, sep="\t")
	...
	if len(regions) > 0:
		...
		for reg in regions:
			...
			pvals_region.to_csv(cfg['file'].replace('.gz','') + '.' + cfg['stat'] + '.rgnl.chr' + reg.replace(':','bp') + '.plotdata',header=True, index=False, sep='\t')
			cmd = cfg['locuszoom'] + ' --metal ' + cfg['file'].replace('.gz','') + '.' + cfg['stat'] + '.rgnl.chr' + reg.replace(':','bp') + '.plotdata --chr ' + str(chr) + ' --start ' + str(start) + ' --end ' + str(end) + ' --plotonly --cache None --prefix ' + cfg['file'].replace('.gz','') + '.' + cfg['stat']
