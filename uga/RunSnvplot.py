...
def RunSnvplot(args):
	...
	print str(r.shape[0]) + " total snvs found"
	...
	for pcol in pcols:
		...
		print "   " + str(results.shape[0]) + " snvs with plottable p-values"
		...
		print "   genomic inflation (all snvs) = " + str(l)
		lA='NA'
		lB='NA'
		lC='NA'
		lD='NA'
		...
		if cfg['mht']:
			print "   generating frequency stratified qq plot for p-value column " + pcol
			if not cfg['nogc']:
			...
			print "   calculating genomic positions for p-value column " + pcol
