...
def Map(file, out, cpu, format, mb = None, kb = None, b = None, n = None):
			...
			with open(out, "w") as fout:
				for i in range(1,27):
					if i in bim['chr'].values:
						...
						if not s is None:
							chr_write=pd.DataFrame({'chr': [i for x in range(1,bim_chr.iloc[-1]['pos']+1,1000000)], 'start': range(1,bim_chr.iloc[-1]['pos']+1,s), 'end': range(s+1,bim_chr.iloc[-1]['pos']+s+1,s)})
